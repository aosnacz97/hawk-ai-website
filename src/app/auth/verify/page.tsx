    ...
    try {
      ...
      if (data.session) {
        ...
        setTimeout(() => {
          ...
          const appUrl = `easeup://auth/verified?session=${encodeURIComponent(JSON.stringify(data.session))}`;
        ...
        setStatus('error');
        setMessage('No valid session found. The verification link may have expired.');
      }
    } catch (error) {
      console.error('Email verification error:', error);
      setStatus('error');
