...
import { getIOSVerificationEmailTemplate, generateIOSDeepLink } from '@/lib/ios-email-templates';
  ...
  try {
    ...
    if (!globalRateLimitResult.allowed) {
      logSecurityEvent('GLOBAL_RATE_LIMIT_EXCEEDED', {
        ...
        endpoint: 'send-verification-ios'
      }, request);
    ...
    const { email, name, iosScheme = 'easeup' } = body;
    ...
    // Create iOS deep link
    const iosDeepLink = generateIOSDeepLink(
      iosScheme,
      'verify-email',
      verificationToken
    );
    ...
    logSecurityEvent('IOS_VERIFICATION_EMAIL_SENT', {
      email: sanitizedEmail,
      tier: emailRateLimitResult.tier,
